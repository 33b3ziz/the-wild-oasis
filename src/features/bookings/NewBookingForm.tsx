...
function NewBookingForm({ onCloseModal }: { onCloseModal?: () => void }) {
  const [wantsBreakfast, setWantsBreakfast] = useState(false);
  const [isPaid, setIsPaid] = useState(false);
  const { cabins, isLoading } = useCabins();
  const { guests, isLoading: isLoadingGuests } = useGuests();
  const { settings, isLoading: isLoadingSettings } = useSettings();
  const { createBooking, isPending: isCreating } = useCreateBooking();
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)}>
      <FormRow label="Start date" error={errors?.startDate?.message as string}>
        <Input
          disabled={isCreating}
          type="date"
          id="startDate"
          {...register("startDate", {
            ...
            validate: isDate(
              getValues().startDate || "You must choose a valid date"
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ) as any,
          })}
        />
      </FormRow>
      <FormRow label="End date" error={errors?.endDate?.message as string}>
        <Input
          disabled={isCreating}
          type="date"
          id="endDate"
          {...register("endDate", {
            required: "This field is required",
            validate: (isDate(getValues().endDate) ||
              "You must choose a valid date") as any,
          })}
        />
      </FormRow>
      <FormRow
        label="Number of guests"
        error={errors?.numGuests?.message as string}
      >
        <Input
          disabled={isCreating}
          type="number"
